...
def exporta_dados_hidro():
    ...
    while True:
        path = str(
            input("""Digite o caminho onde o arquivo será salvo (Exemplo: C:/Temp):""")
        )
        try:
        ...
        if tipo_estacao == TipoDeEstacao.FLUVIOMETRICA:
            for tipo_dados in [TipoDeDados.COTAS, TipoDeDados.VAZOES]:
                ...
                file_path = os.path.join(
                    path, f"serie_{tipo_dados.name}_estacao_{cod_estacao}.csv"
                )
...
def exporta_dados_hidro_por_geometria(
    ...
    print(
        f"Foram identificadas {len(codigo_estacoes_selecionadas)} estações "
